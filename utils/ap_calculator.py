...
from tridetr.utils.box_util import (extract_pc_in_box3d, flip_axis_to_camera_np,
                            get_3d_box, get_3d_box_batch)
from tridetr.utils.eval_det import eval_det_multiprocessing, get_iou_obb
from tridetr.utils.nms import nms_2d_faster, nms_3d_faster, nms_3d_faster_samecls
...
# Inputs are adjusted for use in our training flow.
def parse_predictions(
    ...
    sem_cls_probs = box_preds["sem_cls_prob"].detach().cpu().numpy()  # B,num_proposal,10
    pred_sem_cls_prob = np.max(sem_cls_probs, -1)  # B,num_proposal
    pred_sem_cls = np.argmax(sem_cls_probs, -1)
    obj_prob = box_preds["objectness_prob"].detach().cpu().numpy()

    pred_corners_3d_upright_camera = box_preds["box_corners"].detach().cpu().numpy()
